...
async function writeConvertedMap_test(log = false) {
    ...
    for (const tilesetShape of newTilesetShapes) {
        tilesetShape.source = `.${tilesetShape.source.substring(tilesetShape.source.indexOf("input-output") + 12)}`; //replace everything up to and with "input-output" with .
    }
    const ioDir = await dungeonsApi.readDir();
    for (const file of ioDir) {
        if (file.isFile()) {
            if (getExtension(file.name) === "png") {
                ...
                try {
                    pixelsArray = await getPixelsPromise(`${dungeonsApi.ioDirPath}/${file.name}`);
                }
                catch (error) {
                    console.error(error);
                    return undefined;
                ...
                convertedChunk.addBothTilelayers(convertedFrontLayer, convertedBackLayer, pixelsArray.shape[0], pixelsArray.shape[1]);
