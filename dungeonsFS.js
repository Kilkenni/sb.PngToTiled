...
async function getDungeons(dungeonPath) {
  try {
      const dungeonsRaw = await nodeFileSys.readFile(dungeonPath, { encoding: "utf-8" });
      ;
...
async function writeConvertedDungeons(JsonData) {
  ...
  for(const file of ioDir){
    if(file.isFile())
      ...
      try {
        if(getExtension(file.name) === "dungeon") { //for every .dungeon file in I/O dir
        ...
        if(newDungeonPath) {
          ...
          catch(error) { //this error appears if no .new file is found, i.e. if we can safely write
...
async function writeConvertedMapJson(newPath, JsonData){
  ...
  catch(error) { //this error appears if no converted file is found, i.e. if we can safely write
    // console.log(error.message);
    const template = await getDungeons(`${ioDirPath}/template.json`);
    let fileToWrite = { ...template, ...JsonData};
    await nodeFileSys.writeFile(newPath, JSON.stringify(fileToWrite, null, 2), "utf-8");
    // console.log(`Writing ${file.name}.new done.`);
    return true;
  }  
}
